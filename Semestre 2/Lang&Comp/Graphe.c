...
void GrapheAfficher(graf *pgG,char *sMessage){
	...
	int aK,sX;
	Assert2("GrapheAfficher",bGrapheAmorceR,sMessage!=0);
...
void GrapheAfficher2(graf *pgG,char *sMessage){
	...
	int aK,sX;
	Assert2("GrapheAfficher2",bGrapheAmorceR,sMessage!=0);
	printf("%s%c (%s)\n   ",sMessage,':',sPluriel(pgG->nArcEnTout, "arc"));
	printf(" direct:");
	if (pgG->nArcEnTout>0){
		for (sX=1;sX<pgG->nSommetEnTout+1;sX++){
...
void GrapheAfficher3(graf *pgG,char *sMessage){
	//énumère les arcs de gG et de son graphe inverse sous la forme de couples (origine,destination)
	int aK,sX;
	Assert2("GrapheAfficher3",bGrapheAmorceR,sMessage!=0);
	GrapheAfficher2(pgG,sMessage);
	printf("%s%c (%s)\n   ",sMessage,':',sPluriel(pgG->nArcEnTout, "arc"));
	printf(" cout:");
	if (pgG->nArcEnTout>0){
		for (sX=1;sX<pgG->nSommetEnTout+1;sX++){
	...
	if (pgG->nArcEnTout>0){
		for (sX=1;sX<pgG->nSommetEnTout+1;sX++){
			for (aK=pgG->aHed[sX];aK<pgG->aHed[sX+1];aK++)
				printf(" (%d,%d:%d) ",sX,pgG->sSuk[aK],pgG->nPrix[aK]);
			//printf("\n");
		}
	} else printf(" Aucun arc");
	printf("\n");
...
int bGrapheCheminE(graf gG,int sSource,int sTrappe){
	...
	//bChemine,nSommet soit 1° un booléen vrai ssi sTrappe a bien été atteint depuis sSource et 2° le nbre de sommets de sSource inclus à sTrappe inclus
		...
		while (bWhile( (bChemine=bChemine || (sX==sSource),sX!=(sY=gG.sPer[sX]))) ) {
			nSommet++;
			sX=sY;
		}
	return(bChemine);
	...
	//Appel0("bGrapheCircuite");
		...
		if (pgG->nSommetEnTout>1){//algo de MARIMONT
			...
			DequeAllouer(pgG->nSommetEnTout);//raze;O(nSommetEnTout)
				...
				while (bWhile( (nMarque++,nCard=nDequeCardinal())) ){//nDequeCardinal() est en O(1) et le while en O(A) ——justification en fin de while
...
void GrapheColorer(graf *pgG,int sOrigine,int sDestination,int nCouleur){
	...
	int aK;
	Assert4("GrapheColorer0",bGrapheSommet(pgG,sOrigine), bGrapheSommet(pgG,sDestination), sOrigine!=sDestination, bCroit(0,nCouleur,4));
...
void GrapheCreer(int iSommet,graf **ppgG){
	//si iSommet!=0, alloue un espace pour pgG et,si iSommet>0,ajoute des arcs via GrapheDecrire(); si iSommet est nul,libère l'espace
	int sX;
	FlipFlop(iSommet!=0,&bGrapheCreeR);
	if (iSommet!=0){
...
int bGrapheEulerien(graf *pgG,int szSource,int szTrappe){
	...
	bStrict=(szSource!=0);
	for (sX=1;sX<=pgG->nSommetEnTout;sX++)
		if ( bImporte=(!bStrict || (sX!=szSource) && (sX!=szTrappe)) , bImporte && bImpair(nDegreh[sX]) )
			bEulerien=kF;
	return(bEulerien);
}//bGrapheEulerien
...
void GraphePonderer(graf *pgG){//O(A)
	//intialiser pgG avec des valeurs unitaires pour les 3 champs nCapa,nCout et nPrix
	int aK;
	for (aK=1;aK<=pgG->nArcEnTout;aK++){
		pgG->nCapa[aK]=1;
		pgG->nCout[aK]=1;
...
void GrapheSymetriser(graf *pgG){
	...
	GrapheInverser(pgG);//remplit le champ aInv qui donne la correspondance entre arc direct et arc inverse
